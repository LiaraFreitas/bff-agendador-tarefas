...
public class CronService {
    ...
    public void buscaTarefasProximaHora() {
        String token = login(converteParaResquestDTO());
        log.info("Iniciada a busca de tarefas");
        LocalDateTime horaAtual = LocalDateTime.now();
        LocalDateTime horaFuturaMaisCinco = LocalDateTime.now().plusHours(1);

        List<TarefasDTOResponse> listaTarefas = tarefasService.bucasTarefasAgendadasPorPeriodo(horaAtual, horaFuturaMaisCinco, token);
        ...
        public LoginRequestDTO converteParaResquestDTO(){
